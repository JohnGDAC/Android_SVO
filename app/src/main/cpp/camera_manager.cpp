...
NDKCamera::NDKCamera()
  ...
  if (status == ACAMERA_OK) {
    ...
    exposureTime_ = exposureRange_.value(10);
  } else {
  ...
  if (status == ACAMERA_OK) {
    ...
    sensitivity_ = sensitivityRange_.value(10);
  } else {
...
bool NDKCamera::MatchCaptureSizeRequest(int32_t requestWidth,
  ...
  if (foundIt) {
    compatibleCameraRes_.width = foundRes.org_width();
    compatibleCameraRes_.height = foundRes.org_height();
    if (resCap) {
      resCap->width = maxJPG.org_width();
      resCap->height = maxJPG.org_height();
    }
    LOGI("find the compatible camera resolution, taking %d x %d",
         foundRes.org_width(), foundRes.org_height());
  } else {
    LOGW("Did not find any compatible camera resolution, taking 640 x 480");
    if (disp.IsPortrait()) {
      compatibleCameraRes_.width = 480;
      compatibleCameraRes_.height = 640;
    } else {
      compatibleCameraRes_.width = 640;
      compatibleCameraRes_.height = 480;
    }
    if (resCap) *resCap = compatibleCameraRes_;
  }
  compatibleCameraRes_.format = AIMAGE_FORMAT_YUV_420_888;
...
void NDKCamera::CreateSession(ANativeWindow* previewWindow,
  ...
  CALL_REQUEST(setEntry_i64(requests_[PREVIEW_REQUEST_IDX].request_,
    ...
    CALL_REQUEST(setEntry_u8(requests_[PREVIEW_REQUEST_IDX].request_,
                             ACAMERA_CONTROL_AF_MODE, 1, &afModeOff));
    uint8_t afModeTri = ACAMERA_CONTROL_AF_TRIGGER_IDLE;
    CALL_REQUEST(setEntry_u8(requests_[PREVIEW_REQUEST_IDX].request_,
                             ACAMERA_CONTROL_AF_TRIGGER, 1, &afModeTri));
    float focus = 0.5;
    CALL_REQUEST(setEntry_float(requests_[PREVIEW_REQUEST_IDX].request_,
                                ACAMERA_LENS_FOCUS_DISTANCE, 1, &focus));
    focus = 15;
    CALL_REQUEST(setEntry_float(requests_[PREVIEW_REQUEST_IDX].request_,
                                ACAMERA_LENS_FOCAL_LENGTH, 1, &focus));
