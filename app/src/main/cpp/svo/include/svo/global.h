...
EIGEN_DEFINE_STL_VECTOR_SPECIALIZATION(Eigen::Vector2d)
  ...
  #define SVO_DEBUG_STREAM(fmt, ...) __android_log_print(ANDROID_LOG_DEBUG, "SVO",  "[%s %d] " fmt, FILENAME(__FILE__), __LINE__, ##__VA_ARGS__)
  #define SVO_INFO_STREAM(fmt, ...) __android_log_print(ANDROID_LOG_INFO, "SVO",  "[%s %d] " fmt, FILENAME(__FILE__), __LINE__, ##__VA_ARGS__)
  #define SVO_WARN_STREAM(fmt, ...) __android_log_print(ANDROID_LOG_WARN, "SVO",  "[%s %d] " fmt, FILENAME(__FILE__), __LINE__, ##__VA_ARGS__)
  #define SVO_ERROR_STREAM(fmt, ...) __android_log_print(ANDROID_LOG_ERROR, "SVO",  "[%s %d] " fmt, FILENAME(__FILE__), __LINE__, ##__VA_ARGS__)
  #define SVO_WARN_STREAM_THROTTLE(rate, fmt, ...) \
    do { \
      ...
      if (__log_stream_throttle__last_hit__ + rate <= \
        ...
        std::chrono::duration_cast<std::chrono::seconds>( \
        __log_stream_throttle__now__.time_since_epoch()).count(); \
        SVO_WARN_STREAM(fmt, ##__VA_ARGS__); \
      } \
    } while(0)
