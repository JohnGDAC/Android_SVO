...
calcFromPlaneParams(const Vector3d& n_c1, const Vector3d& xyz_c1)
{
  ...
  H_c2_from_c1 = T_c2_from_c1.rotation_matrix() + (T_c2_from_c1.translation_vec() * n_c1.transpose())/d;
}
...
decompose()
{
  ...
  for(unsigned int i=0; i<decompositions.size(); i++)
  {
    ...
    Quaterniond qua(R);
    decompositions[i].T = SE3(t.x(), t.y(), t.z(),
...
findBestDecomposition()
{
  ...
  else  // two-way ambiguity. Resolve by sampsonus score of all points.
  {
    ...
    for(size_t i=0; i<2; i++)
    {
      ...
      Matrix3d Essential = T.rotation_matrix() * sqew(T.translation_vec());
      double dSumError = 0;
      for(size_t m=0; m < fts_c1.size(); m++ )
      {
