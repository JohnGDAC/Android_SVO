  ...
  for(auto kf : keyframes_)
  {
    ...
    for(auto keypoint : kf->key_pts_)
    {
      ...
      if(frame->isVisible(keypoint->point->pos_))
      {
        close_kfs.push_back(
            std::make_pair(
                kf, (frame->T_f_w_.translation_vec() - kf->T_f_w_.translation_vec()).norm()));
...
void Map::transform(const Matrix3d& R, const Vector3d& t, const double& s)
{
  for(auto it=keyframes_.begin(), ite=keyframes_.end(); it!=ite; ++it)
  {
    ...
    (*it)->T_f_w_ = SE3(pos.x(), pos.y(), pos.z(),
                        q_rot.x(), q_rot.y(), q_rot.z(), q_rot.w()).inverse();
...
/**
 * candidates是深度滤波器中已经收敛的3d点，如果在当前关键帧中首次观测到，加入到当前关键帧中
*/
void MapPointCandidates::addCandidatePointToFrame(FramePtr frame)
{
  ...
  {
    // 该点首次在frame帧观测到，则添加进frame帧的feature集合中，从map的候选点集合中删除该点
