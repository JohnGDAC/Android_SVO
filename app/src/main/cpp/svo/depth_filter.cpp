...
void DepthFilter::initializeSeeds(FramePtr frame)
{
  ...
  if(options_.verbose)
    SVO_INFO_STREAM("DepthFilter: Initialized %zu new seeds", new_features.size());
  seeds_updating_halt_ = false;
}
...
static double normal_pdf(double x, double mean, double std_dev) {
    static const double SQRT_2_PI = 1.41421356237309505;
    double exponent = -0.5 * pow((x - mean) / std_dev, 2);
    return (1 / (std_dev * SQRT_2_PI)) * exp(exponent);
}
...
void DepthFilter::updateSeed(const float x, const float tau2, Seed* seed)
{
  ...
  float C1 = seed->a/(seed->a+seed->b) * normal_pdf(x, seed->mu, norm_scale); //std::pdf(nd, x);
...
{
  Vector3d t;
  t.x() = T_ref_cur.get_translation().x;
  t.y() = T_ref_cur.get_translation().y;
  t.z() = T_ref_cur.get_translation().z;
