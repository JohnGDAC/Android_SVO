...
void FrameHandlerMono::addImage(const cv::Mat& img, const double timestamp)
{
  ...
  new_frame_.reset(new Frame(cam_, img, timestamp));
...
FrameHandlerMono::UpdateResult FrameHandlerMono::processFirstFrame()
{
  // 第一帧位姿初始化，作为世界坐标系
  if(init_pose_)
    new_frame_->T_f_w_ = *init_pose_;
  else
    new_frame_->T_f_w_ = SE3();
...
FrameHandlerBase::UpdateResult FrameHandlerMono::processFrame()
{
  ...
  */
  ...
  SVO_DEBUG_STREAM("Img Align:\t Tracked = %zu", img_align_n_tracked);
  ...
  */
  ...
  SVO_DEBUG_STREAM("Reprojection:\t nPoints = %zu \t \t nMatches = %zu", repr_n_mps, repr_n_new_references);
  ...
  */
  ...
  SVO_DEBUG_STREAM("PoseOptimizer:\t ErrInit = %f px\t thresh = %f", sfba_error_init, sfba_thresh);
  SVO_DEBUG_STREAM("PoseOptimizer:\t ErrFin. = %f px\t nObsFin. = %zu", sfba_error_final, sfba_n_edges_final);
...
void FrameHandlerMono::setCoreKfs(size_t n_closest)
{
  ...
  std::partial_sort(overlap_kfs_.begin(), overlap_kfs_.begin()+n, overlap_kfs_.end(),
                    [](const std::pair<FramePtr, size_t> &l, const std::pair<FramePtr, size_t> &r)
