    ...
    size_t& num_obs)
{
  ...
  for(size_t iter=0; iter<n_iter; iter++)
  {
    ...
    // compute residual
    // 遍历当前帧特征点，计算Jacobian、Residual，构造H * dT = b
    for(auto it=frame->fts_.begin(); it!=frame->fts_.end(); ++it)
    {
      if((*it)->point == NULL)
        continue;
    ...
    if((iter > 0 && new_chi2 > chi2 * 1.2) || (bool) std::isnan((double)dT[0]))
    {
      if(verbose) {
          std::stringstream ss;
          ss << "it " << iter
                    << "\t FAILURE \t new_chi2 = " << new_chi2 << std::endl;
          SVO_WARN_STREAM("%s", ss.str().c_str());
      }
    ...
    // update the model
    // 更新当前帧位姿
    SE3 T_new = SE3::exp(dT.data())*frame->T_f_w_;
    T_old = frame->T_f_w_;
    frame->T_f_w_ = T_new;
    chi2 = new_chi2;
    if(verbose) {
        ...
        SVO_INFO_STREAM("%s", ss.str().c_str());
    }
  ...
  if(verbose) {
      std::stringstream ss;
      ss << "n deleted obs = " << n_deleted_refs
